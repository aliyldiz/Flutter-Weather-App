...
class MyHomePageState extends State<MyHomePage> {
  ...
  int selectedTitleId = 5;
  List<String> mapTitle = [
    'clouds_new',
    'precipitation_new',
    'pressure_new',
    'wind_new',
  ...
  List<String> mapDesc = ['Bulut', 'Yağış', 'Basınç', 'Rüzgar', 'Sıcaklık'];
  ...
  checkFavs() {
    globals.getSharedFunc().then((value) => value.forEach((element) async {
          (WeatherModel, List<WeatherModel>) tempData;
          tempData = await weatherApi.getWeatherData(
              q: WeatherModel.fromJson(jsonDecode(element)).name);
          addItemToList(wtData: tempData.$1);
        }));
  }
  ...
  void addItemToList({WeatherModel? wtData}) {
    WeatherModel? tmpData;
      if (wtData == null) {
        tmpData = weatherData;
        bool haveCity = false;
        ...
        if (!haveCity) {
          globals.favPlaces.insert(0, tmpData!);
        }
      } else {
        tmpData = wtData;
        globals.favPlaces.insert(0, tmpData);
      }
      globals.setSharedFunc(favPlaces);
  }
                                  ...
                                  Opacity(
                                      ...
                                      child: Text(
                                        NumberFormat.decimalPatternDigits(
                                                locale: 'en', decimalDigits: 2)
                                            .format((weatherData?.wind?.speed) *
                                                3.6),
                                        style: const TextStyle(
                                            fontSize: 26,
                                            fontWeight: FontWeight.bold),
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(
                                ...
                                child: Image(
                                    image: AssetImage(
                                        'assets/images/detail/clock.png'))),
                            const Spacer(),
                          ],
                        ),
                      ),
      ...
      child: Column(
        children: [
          Padding(
            ...
            child: Row(
              children: List.generate(
                  5, (i) => mapTile(i + 1, mapTitle[i], mapDesc[i])),
            ),
          ),
          Padding(
  ...
  Widget mapTile(int id, String key, String desc) {
    return Expanded(
      child: InkWell(
        child: Container(
            decoration: BoxDecoration(
                color: selectedTitleId == id ? Colors.white12 : null,
